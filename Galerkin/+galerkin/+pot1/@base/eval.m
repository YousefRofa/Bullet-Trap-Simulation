...
addParameter( p, 'siz', [ n1, n2 ] );
parse( p, varargin{ : } );

%  allocate output
[ data.SL1, data.SL2, data.DL ] = deal( zeros( p.Results.siz ) );
...
for i1 = 1 : numel( obj )
  ...
  if ismember( tau1( 1 ).inout( inout ), tau2( 1 ).inout )
    ...
    fun1 = @( x ) accumarray( { nu1( : ), nu2( : ) }, x( : ), p.Results.siz );
