function [ e, h ] = fields2( obj, sol, varargin )
%  FIELDS - Electromagnetic fields at point positions.
%
%  This is a copy of the file galerkin.pot2.base/fields, however, with the
%  consideration of different fields at the particle inside and outside.
%
%  Usage for obj = galerkin.pot2.base :
%    [ e, h ] = fields2( obj, sol, PropertyPairs )
%  Input
...
addParameter( p, 'name', 'feibelman.fields' );
%  parse input
parse( p, varargin{ : } );

%  tangential fields at boundary inside and outside
sol = toggle( sol, 2 );
ue1 = reshape( sol.et, size( sol.e, 1 ), [] );
uh1 = reshape( sol.ht, size( sol.h, 1 ), [] );
ue2 = reshape( sol.e,  size( sol.e, 1 ), [] );
uh2 = reshape( sol.h,  size( sol.h, 1 ), [] );
...
for it = 1 : numel( obj )
  ...
  %  electromagnetic fields at particle inside or outside
  switch sig
    case 1
      [ ue, uh ] = deal( ue1, uh1 );
    otherwise
