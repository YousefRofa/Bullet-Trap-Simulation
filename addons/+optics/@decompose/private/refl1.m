function [ r, t ] = refl1( layer, k0, dir, einc )
%  REFL1 - Reflected and transmitted fields for upgoing waves.
%
%  Usage :
...
e1 = bsxfun( @times, e1, exp( 2i * k1z * z1 ) );

%  propagation direction for transmitted waves
dir2 = [ k1 * dir( :, 1 : 2 ), k2z ] / k2;
i2 = ~imag( dir2( :, 3 ) );
%  transmitted fields
tm2 = - cross( dir2, cross( dir, tm ) );
e2 = bsxfun( @times, te,  t.te .* dot( te, einc, 2 ) ) +  ...
     bsxfun( @times, tm2, t.tm .* dot( tm, einc, 2 ) ) * Z2 / Z1;
e2 = bsxfun( @times, e2, exp( 1i * ( k1z * z1 - k2z * z2 ) ) );
